...
export default function Projects() {
  return <section id="projects" className="section projects container"><div className="section-heading reveal-on-scroll"><div className="section-label"><span>03</span> Selected work</div><h2>Three brands.<br />Three distinct <em>worlds.</em></h2></div><div className="projects-list">{projects.map((project, index) => <article className="project-card reveal-on-scroll" key={project.name}><ProjectVisual project={project} number={`0${index + 1}`} /><div className="project-details"><div className="project-number">0{index + 1} / 03</div><small>{project.category}</small><h3>{project.name}</h3><p>{project.description}</p><ul>{project.features.map((feature) => <li key={feature}>{feature}</li>)}</ul><a className="project-link" href={project.url} target="_blank" rel="noreferrer">Explore live project <span>↗</span></a></div></article>)}</div></section>;
}
